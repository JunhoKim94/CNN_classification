...
model = Conv_LM(embed, h, len(ch_corpus), hidden, len(word2idx), num_layer, drop_out)
...
for epoch in range(epochs):
    ...
    for iteration in range(total // batch_size):
        ...
        #hidden = (torch.zeros(1, batch, 300).to(device), torch.zeros(1,batch,300).to(device))
    ...
    val_h = (torch.zeros(1,batch_size,300).to(device), torch.zeros(1,batch_size,300).to(device))
    ...
    val_loss = criterion(y_val, val_y) * batch_size

    val_ppl = torch.exp(val_loss.data)

    if (epoch % 1 == 0):
        print(f"epoch = {epoch} | PPL = {PPL} | Val_PPL = {val_ppl} | lr = {lr} | spend time : {time.time() - st}")
